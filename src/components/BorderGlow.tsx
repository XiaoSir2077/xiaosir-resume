...
function parseHSL(hslStr: string): HSL {
  ...
  if (!match) return { h: 24, s: 95, l: 60 }; // Default to orange!
...
const BorderGlow: React.FC<BorderGlowProps> = ({
  ...
  className = '',
  edgeSensitivity = 30,
  glowColor = '24 95 60', // Default to beautiful orange glow (Hue 24, Saturation 95%, Lightness 60%)
  backgroundColor = '#020617', // Match slate-950 background
  borderRadius = 16, // Default to a standard card radius
  glowRadius = 40,
  glowIntensity = 1.0,
  coneSpread = 25,
  animated = false,
  colors = ['#f97316', '#f59e0b', '#ef4444'], // Default to beautiful warm gradient colors (orange, amber, red)
  ...
  useEffect(() => {
    ...
    // Helper to update pointer position from angle during sweep
    const updatePointer = (ang: number) => {
      const rect = card.getBoundingClientRect();
